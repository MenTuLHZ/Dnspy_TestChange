...
namespace dnSpy.Debugger.DotNet.Evaluation.Engine.Interpreter {
	abstract class DbgDotNetILInterpreter {
		...
		public DbgDotNetValueResult Execute(DbgEvaluationInfo evalInfo, DbgDotNetILInterpreterState state, string typeName, string methodName, DbgEvaluationOptions options, out DmdType expectedType) {
			...
			DbgDotNetValueResult result = Execute(evalInfo, genericTypeArguments, genericMethodArguments, null, null, state, typeName, methodName, options, out expectedType);
			//DbgDotNetRawValue test = result.Value.GetRawValue().RawValue?;
			//test.RawValue
			//if (frameMethod != null && result.Value != null) {
			//	Test_Print_Log.Test_Print_Log.Log_Something(frameMethod.ToString(), (result.Value.GetRawValue().RawValue != null) ? result.Value.GetRawValue().RawValue.ToString() : "");
			//}
