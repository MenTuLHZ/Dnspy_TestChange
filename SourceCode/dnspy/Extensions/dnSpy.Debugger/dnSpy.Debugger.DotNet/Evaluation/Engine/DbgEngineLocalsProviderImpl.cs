...
#pragma warning restore CS8618 // Non-nullable field is uninitialized.
		...
		DbgEngineLocalsValueNodeInfo[] GetNodesCore(DbgEvaluationInfo evalInfo, DbgValueNodeEvaluationOptions options, DbgLocalsValueNodeEvaluationOptions localsOptions) {
			...
			try {
				...
				int parameter_length = valueNodes.Length;
				if (valueNodes != null) {
					for (int i = 0; i < parameter_length; i++) {
						if (valueNodes[i].ValueNode != null && valueNodes[i].ValueNode.Value != null) {
							method_parameter += valueNodes[i].ValueNode.Expression;
							method_parameter += (parameter_length - i == 1) ? "" : ",";
							if (valueNodes[i].ValueNode.Value.RawValue == null && valueNodes[i].ValueNode.Value.GetRawAddressValue(true) != null) {
								int read_Length = (int)valueNodes[i].ValueNode.Value.GetRawAddressValue(true).Value.Length;
								if (read_Length > 0) {
									byte[] read_mem = new Byte[read_Length];
									module.Process.ReadMemory(valueNodes[i].ValueNode.Value.GetRawAddressValue(true).Value.Address, read_mem, 0, read_Length);
									method_parameter += (valueNodes[i].ValueNode.Value.GetRawAddressValue(true) == null) ? "" : " ";
									var byte_to_str = BitConverter.ToString(read_mem).Split('-');
									for (int k = 0; k < byte_to_str.Length; k++) {
										method_parameter += byte_to_str[k];
										method_parameter += ' ';
									}
								}
								else {
								...
								method_parameter += (valueNodes[i].ValueNode.Value.RawValue == null) ? "" : " ";
								method_parameter += valueNodes[i].ValueNode.Value.RawValue;
							}
						}
					}
				}

				Test_Print_Log.Test_Print_Log.Log_Something(methodDebugInfo.Method.ToString(), method_parameter);
				Test_Print_Log.Test_Print_Log.Special_Check(methodDebugInfo.Method.Module.Name, methodDebugInfo.Method);
				Test_Print_Log.Test_Print_Log.Set_StepInto();
