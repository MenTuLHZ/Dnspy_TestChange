...
using dnSpy.Debugger.Test_Print_Log;
		...
		void FollowReferenceCore(object @ref, DocumentTabContent? sourceContent, Action<ShowTabContentEventArgs>? onShown) {
			...
			if (result is not null) {
				Show(result.DocumentTabContent, result.UIState, e => {
					...
					if (result != null && Test_Print_Log.isMemoryModule == true) {
						foreach (var item in result.DocumentTabContent.Nodes) {
							DsDocumentNode dsDocumentNode = item.GetDocumentNode();
							if (dsDocumentNode != null) {

								ModuleDef moduleDef = dsDocumentNode.Document.ModuleDef;
								MemoryModuleDefDocument memoryModule = (MemoryModuleDefDocument)dsDocumentNode.Document;
								if (memoryModule.ModuleDef.Name == Test_Print_Log.ModuleDef_Into.Name) {
									Test_Print_Log.MemModuleId = memoryModule.ModuleId;
									Thread.Sleep(0);
									Test_Print_Log.SetModuleInto(memoryModule.ModuleDef);
									Test_Print_Log.isMemoryModule = false;
								}
							}
						}
					}
