			...
			try {
				...
				if (isDotNet) {
					try {
						...
						ModuleDef moduleDef = ModuleDefMD.Load(peImage, options);
						if (moduleDef.EntryPoint != null) {
							Test_Print_Log.ModuleDef_Into = moduleDef;
						}
						return DsDotNetDocument.CreateAssembly(documentInfo, moduleDef, true);
					}
					catch {
					}
				}
